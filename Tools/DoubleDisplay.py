...
def ToolRun_DoubleDisplay():
    ...
    def open_window(loaded):
        ...
        while True:
            ...
            if event2 == "Load":
                ...
                break
        window2.close()
        return (img_list1, img_list2, status)
    ...
    window = sg.Window('Double Image Display', layout, return_keyboard_events=True, use_default_focus=False, finalize=True, resizable=True)
    ...
    while True:
        ...
        if event == 'Load Excel':
            ...
            img_compare = []
            img_list1, img_list2, loaded_files = open_window(loaded_files)
            if loaded_files:
                global_list = []
        ...
        if loaded_files:
            if (event == 'document_list') or (event == 'Next') or (event == 'Previous') or (event == 'document_list_Down') or (event == 'document_list_Up'):
                img_compare = []
                if event == "doc1":
                ...
                if excel_conversion:
                    justification = df.iloc[df[df['Conflicting DocId'] == document2].index[0], df.columns.get_loc('Manual Remarks')]
                    note = df.iloc[df[df['Conflicting DocId'] == document2].index[0], df.columns.get_loc('Notes')]
                    details = df.iloc[df[df['Conflicting DocId'] == document2].index[0], df.columns.get_loc('More Details')]
